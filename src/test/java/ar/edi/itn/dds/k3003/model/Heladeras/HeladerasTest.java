...
void testDepositar() {
    ...
    assertEquals("33",repo.findById(1).getViandas().get(0));
    assertEquals("22",repo.findById(2).getViandas().get(0));
    assertEquals("44",repo.findById(2).getViandas().get(1));
 }
...
void testRetirarVianda() {
    ...
    assertEquals("44",repo.findById(2).getViandas().get(0));//la vianda que quedo en la heladera 2
...
void obtenerTemperatura(){
    ...
    assertEquals(tempDto2.getTemperatura(),repoT.findById(1).get(1).getTemperatura());
    assertEquals(2,repoT.findById(1).size());
    }
}
